...
the same function as the Arduino UNO board.
  In addition it should be noted that it already has a built-in Wifi module. 
...
******************** Connection and Sending data to ThingSpeak *******************
String data = String("field1=" + String(sensorValue, DEC));     // POST command to send.
if (client.connect(server, 80))  {                             // Connexion with the port 80.
...
******************** Connection and Sending data to Raspberry ******************
if(client.connect(server2, 80)) {             // Connexion with the port 80.
// Display all the data.
